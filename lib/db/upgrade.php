...
function xmldb_main_upgrade($oldversion) {
    ...
    // This is done in four parts to aid error recovery, if it occurs, during upgrade.
    if ($oldversion < 2018070600.01) {
        $table = new xmldb_table('question');
        $field = new xmldb_field('idnumber', XMLDB_TYPE_CHAR, '100', null, null, null, null, 'modifiedby');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        upgrade_main_savepoint(true, 2018070600.01);
    }

    if ($oldversion < 2018070600.02) {
        $table = new xmldb_table('question');
        $index = new xmldb_index('categoryidnumber', XMLDB_INDEX_UNIQUE, array('category, idnumber'));
        if (!$dbman->index_exists($table, $index)) {
            $dbman->add_index($table, $index);
        }
        upgrade_main_savepoint(true, 2018070600.02);
    }

    if ($oldversion < 2018070600.03) {
        $table = new xmldb_table('question_categories');
        $field = new xmldb_field('idnumber', XMLDB_TYPE_CHAR, '100', null, null, null, null, 'sortorder');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        upgrade_main_savepoint(true, 2018070600.03);
    }

    if ($oldversion < 2018070600.04) {
        $table = new xmldb_table('question_categories');
        ...
        // Main savepoint reached.
        upgrade_main_savepoint(true, 2018070600.04);
    }
