...
class theme_config {
    ...
    /**
     * The list of URLs queued to be fetched lazily within the page.
     * @var array
     */
    protected $queuedcssurls = [];
    ...
    public function css_urls(moodle_page $page) {
        ...
        if ($rev > -1) {
            ...
            if ($this->get_fallback_css_file() && !$this->is_theme_compiled($rev, $filename)) {
                ...
                // The original URL is queued to be lazily loaded by JS.
                $this->queuedcssurls[] = $url;
            } else {
                $urls[] = $url;
            }
    ...
    /**
     * Get the list of URLs queued for lazy loading.
     *
     * @return moodle_url[]
     */
    public function queued_css_urls() {
        return $this->queuedcssurls;
    }
     ...
     *
     * @param   bool $rtl Whether to fetch the RTL version of the fallback CSS
     * @return  string
     */
