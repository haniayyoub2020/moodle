...
/**
 * Shortens a given array of strings by removing characters positioned after the ideal string length.
 *
 * @param array $path The paths to reduce the length.
 * @param int $length Ideal string length
 * @return array $result Shortened paths in array.
 */
function shorten_filename_from_array(array $path, $length = MAX_FILENAME_SIZE) {
    $result = null;

    $result = array_reduce($path, function($carry, $singlepath) use ($length) {
        $carry[] = shorten_filename($singlepath, $length);
        return $carry;
    }, []);
