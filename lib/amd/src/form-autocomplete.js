...
function($, log, str, templates, notification, LoadingIcon) {
    ...
    var addNavigation = function(options, state, originalSelect) {
        ...
        var refocusFunction = function(state, pendingPromise) {
        ...
        var checkScrollBarFocusFunction = function() {
            if ($(document.activeElement).is(document.getElementById(state.suggestionsId))) {
                // Immediately focus the input element if the scrollbar was selected on the suggestions list.
                inputElement.focus();
            } else {
                // Otherwise check and close after a period.
                window.setTimeout(refocusFunction, 500, state, addPendingJSPromise('form-autocomplete-blur'));
            }
        };

        inputElement.on('blur', function() {
            window.setTimeout(checkScrollBarFocusFunction);
        });
