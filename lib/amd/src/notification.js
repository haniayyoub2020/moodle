...
function(Y, $, log) {
    var notificationModule = {
        ...
        exception: function(ex) {
            if (typeof ex === "string") {
                // The exception provided was a string. It should be an Error, on a Object containing a PHP Exception.
                // Due to a bug, the actual Error can only be included as a second param but should be used in
                // preference over the String version if appropriate.
                if (arguments[1] instanceof Error || arguments[1] instanceof Object) {
                    // A JavaScript Error, or a PHP Exception in an Object.
                    ex = arguments[1];
                } else {
                    // No better option. Normalise the provided string.
                    ex = {name: ex};
                }
            }
            ...
            if (ex.stacktrace) {
                ...
                var ln = ex.stacktrace.match(/line ([^ ]*) of/);
                var fn = ex.stacktrace.match(/ of ([^:]*): /);
                if (ln && ln[1]) {
                    ex.lineNumber = ln[1];
                }
                if (fn && fn[1]) {
