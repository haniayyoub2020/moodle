...
EOF;
    ...
    public function look_for_exceptions() {
        self::look_for_exceptions_in_session($this->getSession());
    }
     ...
     * @see     Moodle\BehatExtension\Tester\MoodleStepTester
     */
    public static function look_for_exceptions_in_session(Session $session) {
    ...
    protected function get_debug_text($html) {
        return self::get_debug_text_from_content($html);
    }
     ...
     * @return  string
     */
    protected static function get_debug_text_from_content(string $html): string {
    ...
    protected function execute($contextapi, $params = array()) {
        self::execute_step($this->getSession(), $contextapi, $params);
    }
     ...
     * @throws  Exception
     */
    public static function execute_step(Session $session, $contextapi, $params = []): void {
        if (!is_array($params)) {
            $params = array($params);
        }
        ...
        // Wait for pending js.
        static::wait_for_pending_js_in_session($session);

        // Look for exceptions.
        static::look_for_exceptions_in_session($session);
    }
