...
function css_store_css(theme_config $theme, $csspath, $csscontent, $chunk = false, $chunkurl = null) {
    ...
    if ($chunk) {
        // The fallback URL is identical except that the type is fallback instead of all.
        $fallbackurl = preg_replace('#/all$#', '/fallback', $chunkurl);
        $fallbackurl = str_replace('type=all', 'type=fallback', $fallbackurl);
        ...
        foreach ($css as $content) {
            ...
            $fallbackcontent = str_replace($chunkurl, $fallbackurl, $content);
