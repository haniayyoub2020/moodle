...
class controller extends controller_base {
    ...
    /** @var The file area for the image associated with a badge */
    const FILEAREA_BADGEIMAGE = 'badgeimage';

    /** @var The file area associated with an awarded badge instance */
    const FILEAREA_USERBADGE = 'userbadge';
    ...
    protected static function get_file_areas(): array {
        return array_merge(parent::get_file_areas(), [
            ...
            self::FILEAREA_BADGEIMAGE => self::ITEMID_PRESENT_IN_USE,
            self::FILEAREA_USERBADGE => self::ITEMID_PRESENT_IN_USE,
        ]);
    }
    ...
    public static function can_access_storedfile(stored_file $file, ?object $user): bool {
        ...
        if ($filearea === self::FILEAREA_BADGEIMAGE) {
        ...
        if ($filearea === self::FILEAREA_USERBADGE) {
    ...
    /**
     * Force download of user badges rather than allowing hosting directly in-browser.
     *
     * @return  null|bool
     */
    protected function should_force_download(): ?bool {
        if ($this->filearea === self::FILEAREA_USERBADGE) {
            return true;
        }
