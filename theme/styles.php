...
if ($type === 'editor') {
    ...
    if ($cache) {
        ...
        css_send_uncached_css(file_get_contents($candidatesheet), $etag);
    }
...
function theme_server_fallback_content($theme, $rev) {
    ...
    if (!$theme->usefallback) {
        // This theme does not use fallbacks.
        return false;
    }
    ...
    $checkfiles = [
        "{$CFG->tempdir}/theme/{$theme->name}/{$filename}",
    ];

    foreach ($checkfiles as $fallbacksheet) {
