...
class initiate_data_request_task extends adhoc_task {
    ...
    public function execute() {
        ...
        mtrace('Generating user request data...');
        api::update_request_status($requestid, api::DATAREQUEST_STATUS_PREPROCESSING);

        // TODO: Add code here to process the request and prepare the context list for review.
        $manager = new \core_privacy\manager();
        $contextcollection = $manager->get_contexts_for_userid($datarequest->get('userid'));

        // When the preparation of the request data finishes, update the request status to awaiting approval.
        api::update_request_status($requestid, api::DATAREQUEST_STATUS_AWAITING_APPROVAL);
        mtrace('User request data generation complete...');
