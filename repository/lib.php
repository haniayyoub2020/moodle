...
abstract class repository implements cacheable_object {
    ...
    public function import_external_file_contents(stored_file $file, $maxbytes = 0) {
        if (!$file->is_external_file()) {
            ...
            // If a file has been downloaded, the file record should report both a positive file size, and a contenthash
            // which does not related to empty content.
            // If thereis no file size, or the contenthash is for an empty file, then the file has yet to be successfully downloaded.
            $contentexists = $this->get_filesize() && $file->get_contenthash() !== sha1('');
