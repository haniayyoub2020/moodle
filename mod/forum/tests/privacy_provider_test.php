 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_forum_privacy_provider_testcase extends \core_privacy\phpunit\provider_testcase {

    // Include the privacy helper trait.
    // This includes the subcontext builders.
    use \mod_forum\privacy\helper;
     ...
     */
    protected function assertDiscussionData($expected, $actual) {
        // Exact matches.
        $this->assertEquals($expected->name, $actual->name);
        $this->assertEquals($expected->pinned, $actual->pinned);

        // TODO Confirm structure of remaining fields.
    }

    /**
     * Helper to assert that the post data is correct.
     */
    protected function assertPostData($expected, $actual, $writer) {
        // Exact matches.
    ...
    public function test_user_has_never_posted_subscribed_to_forum() {
        ...
        // There should be no other data exported.
    ...
    public function test_user_has_never_posted_subscribed_to_discussion() {
        ...
        $this->assertCount(0, $writer->get_data($forumsubcontext));
        ...
        $this->assertDiscussionData($discussion, $data);
    ...
    public function test_user_has_posted_own_discussion() {
        ...
        $this->assertDiscussionData($discussion, $writer->get_data($this->get_subcontext($forum, $discussion)));
        $this->assertPostData($post, $writer->get_data($this->get_subcontext($forum, $discussion, $post)), $writer);
    }
    ...
    public function test_user_has_posted_reply() {
        ...
        $reply = $this->helper_post_to_discussion($forum, $discussion, $user);
        ...
        $this->assertEmpty($writer->get_data($this->get_subcontext($forum, $discussion, $post)));

        // The reply will be included.
        $this->assertPostData($reply, $writer->get_data($this->get_subcontext($forum, $discussion, $reply)), $writer);
    }
    ...
    public function test_user_has_rated_others() {
        ...
        // The original post will not be included.
        $this->assertEmpty($writer->get_data($this->get_subcontext($forum, $discussion, $post)), $writer);
    }
    ...
    public function test_user_forum_digest() {
        ...
        $this->assertEquals(0, \core_privacy\request\writer::with_context($context0)->get_metadata([], 'digestpreference'));

        $this->export_context_data_for_user($user->id, $context1, 'mod_forum');
        $this->assertEquals(1, \core_privacy\request\writer::with_context($context1)->get_metadata([], 'digestpreference'));

        $this->export_context_data_for_user($user->id, $context2, 'mod_forum');
        $this->assertEquals(2, \core_privacy\request\writer::with_context($context2)->get_metadata([], 'digestpreference'));
    }

    /**
     * Test that the per-user, per-forum user tracking data is stored.
     */
    public function test_user_tracking_data() {
        ...
        $this->assertEquals(0, \core_privacy\request\writer::with_context($contextoff)->get_metadata([], 'trackreadpreference'));
    }
    ...
    public function test_user_read_posts() {
        ...
        // Forum 1.
        $this->export_context_data_for_user($user->id, $context1, 'mod_forum');
        $writer = \core_privacy\request\writer::with_context($context1);
        ...
        // Forum 2.
        $this->export_context_data_for_user($user->id, $context2, 'mod_forum');
        $writer = \core_privacy\request\writer::with_context($context2);
        ...
        $writer = \core_privacy\request\writer::with_context($context3);
